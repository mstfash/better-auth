...
* Use `resource` indicators to restrict tokens to an audience ([RFC 8707](https://www.rfc-editor.org/rfc/rfc8707.html)).
...
When the JWT plugin is enabled, an authorized `resource` becomes the access token's `aud` claim. With `disableJwtPlugin: true`, resources remain bound to opaque access and refresh tokens and are exposed by introspection and `customAccessTokenClaims`.
...
Use the `resources` field in `customAccessTokenClaims` to add claims based on the authorized audience.
