...
describe("oauth-provider resource indicator binding", async () => {
	...
	it("persists the authorized resources with user consent", async ({
		onTestFinished,
	}) => {
		const consentClient = await auth.api.adminCreateOAuthClient({
			headers,
			body: { redirect_uris: [redirectUri], skip_consent: false },
		});
		expect(consentClient?.client_id).toBeDefined();
		...
		const url = await createAuthorizationURL({
			...
			redirectURI: "",
			authorizationEndpoint: `${authServerBaseUrl}/api/auth/oauth2/authorize`,
			state: "rfc8707-consent",
			scopes: ["openid", "offline_access"],
			codeVerifier,
		});
		url.searchParams.append("resource", audienceA);
		url.searchParams.append("resource", audienceB);
		...
		expect(result.url).toContain(`${redirectUri}?code=`);

		const context = await auth.$context;
		const consent = await context.adapter.findOne<OAuthConsent<Scope[]>>({
			model: "oauthConsent",
			where: [
				{ field: "clientId", value: consentClient.client_id },
				{ field: "userId", value: user.id },
			],
		});
		expect(consent?.resources).toEqual([audienceA, audienceB]);
	});
});
